...
def generate_commit_message(diff_summary, diff_text, strict=False):
    """diff から提出のタイトル (1行要約 + 箇条書き) を生成する.

    1 行目が PR タイトル (承認タブの見出し・リリースノートの 1 行目) になる。
    """
    prompt = (
        'あなたは構造設計ツール mgtkit のリポジトリ管理を手伝っています。\n'
        '以下の変更内容から、日本語のコミットメッセージを作成してください。\n'
        '形式: 1行目に50字以内の要約、空行、変更点の箇条書き(最大5項目)。\n'
        '1行目はメンバーの一覧画面と、正式版のリリースノートの見出しに\n'
        'そのまま出ます。利用者 (構造設計者) が読んで何が変わるか分かる\n'
        '体言止めにし、箇条書きの記号・ファイル名・接頭辞は付けないで\n'
        'ください。例: 荷重分布図の PDF 書き出しに対応\n'
